    ...
    public partial class frmVenta : Form
    {
        ...
        private void Calculos()
        {
            if(txtPrecio.Text != "")
            {
                ...
                IvaActual = Convert.ToDecimal(txtIvaActual.Text);
                Precio = Convert.ToDecimal(txtPrecio.Text);
                IVA = ((Precio * IvaActual) / 100);
        ...
        {
            DataTable Datos = CapaNegocio.NEmpleado.Mostrar();
            frmPrincipal frm = new frmPrincipal();
            Cargo = frm.Cargo = Datos.Rows[Idempleado][10].ToString();
            frm.GestionUsuarios();
        ...
        public  void chbIva_CheckedChanged(object sender, EventArgs e)
        {
           
            MessageBox.Show("Tipo empleado actual es :"+Cargo);

            if (chbIva.Checked && Cargo== "Gerente")

            {
                this.txtIvaActual.Enabled = true;
               
            }
            else
            {
                MessageBox.Show("Usted debe ser Gerente para realizar esta Opcion", "Sistema de ventas", MessageBoxButtons.OK, MessageBoxIcon.Information);
